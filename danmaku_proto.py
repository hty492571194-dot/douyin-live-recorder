# -*- coding: utf-8 -*-
"""抖音直播弹幕 protobuf 消息定义(betterproto)。

vendored from: https://github.com/saermart/DouyinLiveWebFetcher (AGPL-3.0, protobuf/douyin.py)
仅个人本地使用;仅保留本模块用到的消息类型所需的完整文件。
"""
...
class GiftInfo(betterproto.Message):
    """轻礼物精简信息(WebcastLightGiftMessage.gift_info)"""

    gift_id: int = betterproto.uint64_field(1)
    gift_icon: "Image" = betterproto.message_field(2)
    diamond_count: int = betterproto.uint64_field(3)


@dataclass
class GiftLiteTrayInfo(betterproto.Message):
    """轻礼物托盘(WebcastLightGiftMessage.tray_info)"""
...
class LightGiftMessage(betterproto.Message):
    """轻礼物消息(抖音新版礼物推送,替代观众端 WebcastGiftMessage)。

    注意:送礼人不在顶层 user 字段,而在 common.user(field 15);
    礼物名需取 gift_struct.name(完整结构),gift_info 只有 id/图标/钻石数。
    """

    common: "Common" = betterproto.message_field(1)
    group_count: int = betterproto.uint64_field(2)
    repeat_count: int = betterproto.uint64_field(3)
    combo_count: int = betterproto.uint64_field(4)
    to_user_id: int = betterproto.uint64_field(5)
    priority: "GiftIMPriority" = betterproto.message_field(6)
    gift_info: "GiftInfo" = betterproto.message_field(7)
    tray_info: "GiftLiteTrayInfo" = betterproto.message_field(8)
    send_type: int = betterproto.uint64_field(9)
    count: int = betterproto.uint64_field(10)
    diy_item_info: str = betterproto.string_field(11)
    banned_display_effects: int = betterproto.uint64_field(12)
    gift_struct: "GiftStruct" = betterproto.message_field(13)
